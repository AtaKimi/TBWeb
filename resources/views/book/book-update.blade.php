...
@section('content')
    <div class="container">
        <div class="card bg-light m-auto w-75 pb-3">
            <div class="card-body">
                <h2>Form</h2>
                <form class="row g-3" method="POST" action="/books/{{ $book->uuid }}/update">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group  col-6">
                        ...
                        <input type="text" class="form-control" name="photo" id="photo"
                            placeholder="Enter photo link" value="{{ $book->photo }}" />
                        @error('photo')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="form-group ">
                        ...
                        <select id="book_kind_id" name="book_kind_id" class="form-control">
                            @foreach ($book_kinds as $book_kind)
                                <option value="{{ $book_kind->id }}" @if ($book_kind->id == $book->book_kind_id) selected @endif>
                                    {{ $book_kind->name }}</option>
                            @endforeach
                        </select>
                        @error('book_kind')
                            <p class="text-danger">{{ $message }}</p>
                        ...
                        @foreach ($book_identities as $book_identity)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" value="{{ $book_identity->id }}"
                                    ...
                                    @if ($book->book_identity->contains($book_identity)) checked @endif>
